...
prepare_input_data_kfold_grid <- function(occ_grid_path,clim_grid_path,number_of_folds,maxent_result_dir,saving_footnote=""){
  # input:
  ...
  num_occurrence <- nrow(occ_grid_historical_values)
  # the whole area 
  ...
  background_grid_historical_values <- diff_set[sample(nrow(diff_set), num_occurrence), ]
  ...
  saveRDS(list_to_return, file = paste(maxent_result_dir,"/kfold_input_data_",this_bug,saving_footnote,".RDS",sep = ''))
  endTime <- Sys.time()
...
prepare_input_data_kfold_buffer_grid <- function(occ_grid_path,clim_grid_path,buffer_grid_path,number_of_folds,maxent_result_dir,saving_footnote=""){
  # input:
  # occ_raw_path: the path of the occurence data (in .csv format, with decimal longitude and latitude columns have column names 'DecimalLon',"DecimalLat")
  # clim: the stack of all input training raster data (after reprojection and alignment)
  # number_of_folds: specify the number of folds to split the data (for k-fold cross validation)
...
run_maxent_model_cv_grid <- function(list_x_train_full,list_x_test_full,list_pa_train,list_pa_test,list_p_train,list_a_train,list_p_test,list_a_test,maxent_evaluate_dir,number_replicate,maxent_model_dir,metric_saving=T,model_saving=T,saving_footnote=""){
  ...
  if (metric_saving){
    csv_saving_path <- paste(maxent_evaluate_dir,'/cv_metric_results',saving_footnote,'.csv',sep = '')
    write.csv(metric_result_list, file = csv_saving_path)
    rds_saving_path <- paste(maxent_evaluate_dir,'/cv_metric_results',saving_footnote,'.RDS',sep = '')
    saveRDS(metric_result_list, file = rds_saving_path)
  }
  
  if (model_saving){
    model_saving_path <- paste(maxent_evaluate_dir,'/cv_models',saving_footnote,'.RDS',sep = '')
...
run_maxent_model_training_basic_grid <- function(maxent_evaluate_dir,final_x_train,final_x_test,final_pa_train,final_pa_test,final_p_train,final_a_train,final_p_test,final_a_test,maxent_model_dir,metric_saving=T,model_saving=T,saving_footnote=""){
  ...
  if (metric_saving){
    csv_saving_path <- paste(maxent_evaluate_dir,'/basic_final_metric_results_training_basic',saving_footnote,'.csv',sep = '')
    write.csv(this_metric_list, file = csv_saving_path)
    rds_saving_path <- paste(maxent_evaluate_dir,'/basic_final_metric_results_training_basic',saving_footnote,'.RDS',sep = '')
    saveRDS(this_metric_list, file = rds_saving_path)
  }
  
  if (model_saving){
    # also save the model
    maxent_model_path <- paste(maxent_model_dir,'/basic_final_model_training_basic',saving_footnote,'.RDS',sep = '')
...
run_maxent_model_training_all_grid <- function(maxent_evaluate_dir,all_x_full,all_pa,maxent_result_path,dir_sub_name,maxent_model_dir,model_saving=T,saving_footnote=""){
  ...
  if (model_saving){
    # also save the model
    maxent_model_path <- paste(maxent_model_dir,'/',dir_sub_name,'_final_model_training_all',saving_footnote,'.RDS',sep = '')
    print("test")
...
this_bug = 'San'
number_replicate = 10
top_file_dir = "/Users/vivianhuang/Desktop/R-modeling-scripts/r_chagasM/output/rf/kfold_grid"
...
########################################
#   Q: how many runs to average from?  #
########################################
...
for (i in max_num_runs) {
  ...
  list_cv_tss <- c(list_cv_tss,cv_result_list$tss)
  list_cv_auc <- c(list_cv_auc,cv_result_list$auc)
  list_cv_bias <- c(list_cv_bias,cv_result_list$bias)
  list_cv_mae <- c(list_cv_mae,cv_result_list$mae)
  list_cv_threshold <- c(list_cv_threshold,cv_result_list$tss_threshold)
}
